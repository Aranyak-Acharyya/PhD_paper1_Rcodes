...
save(risk4_vec,file="risk4.RData")




#df1<-data.frame(n_vec,risk1_vec,risk2_vec,risk4_vec)
#print(df1)
#df1m<-melt(df1, id.var='n_vec')
#print(df1m)
#ggplot(data = df1m, aes(x=n_vec, y=value, colour= variable)) +
 # geom_line() +
  #geom_point() +
  #ggtitle("Consistency on known linear manifold") +
  #scale_colour_discrete(
   # labels=unname(TeX(c("values of $\\hat{\\beta}_{true}$",
     #                   "values of $\\hat{\\beta}_{naive}$",
      #                  "values of $\\hat{\\beta}_{adj,\\hat{\\sigma}}$"
    #)
   # ))) +
  #labs(y="sample MSE of estimators",x="no. of nodes") +
